...
channel_url = 'http://www.dinostream.pw/channel.php?file=%s&width=650&height=400&autostart=true'
jquery = '%s?callback=jQuery1704538611799614748_%s&v_cod1=%s&v_cod2=%s&_=%s'
final_rtmp = '%s app=vod/?token=%s playpath=%s flashver=WIN%5C2017,0,0,134 \
...
def verdirect(chnum):
    ...
    if res and 'redirect' in res[1]:
        ...
        rtmp = 'rtmp://185.39.11.26:1735/' #token[0]
    else:
        token = res[1].replace('\\','').split('vod/?')
        rtmp = 'rtmp://185.39.9.34:1735/' #token[0]
    app = 'vod?'+token[1]
